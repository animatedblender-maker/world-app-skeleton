...
function pickContinent(props) {
    const raw = props?.CONTINENT ??
        props?.continent ??
        props?.REGION_UN ??
        props?.REGION_WB ??
        props?.SUBREGION ??
    ...
    if (!s)
        return 'Unknown';
    if (s.includes('North America'))
        return 'North America';
    if (s.includes('South America'))
        return 'South America';
    if (s.includes('Europe'))
        return 'Europe';
    if (s.includes('Africa'))
        return 'Africa';
    if (s.includes('Asia'))
        return 'Asia';
    if (s.includes('Oceania') || s.includes('Australia'))
        return 'Oceania';
    if (s.includes('Antarctica'))
        return 'Antarctica';
    return s;
}
function pushRing(points, ring) {
...
function loadCountriesFromGeoJSON() {
    if (cachedCountries)
        return cachedCountries;
    if (!fs.existsSync(GEOJSON_PATH)) {
        throw new Error(`GeoJSON not found at ${GEOJSON_PATH}
