...
export class LocationService {
  ...
  private async fromClientIp(clientIp?: string | null): Promise<DetectedLocation | null> {
    const ip = this.normalizeIp(clientIp);
    // Prefer ip-api (no key, good geo). Fall back to ipapi.co.
  ...
  private async fromIpApi(ip: string | null): Promise<DetectedLocation | null> {
    try {
      const path = ip
  ...
  private async fromIpApiCo(ip: string | null): Promise<DetectedLocation | null> {
    try {
      const path = ip
