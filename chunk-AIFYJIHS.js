import{a as q}from"./chunk-LJDR2ADK.js";import{a as L}from"./chunk-7DJA5TI4.js";import{a as y}from"./chunk-LQQHCQP3.js";import{b as k}from"./chunk-KO5X6LCN.js";import{a as f,k as b,n as P}from"./chunk-SPQ2JWKS.js";var D=class d{createdPostSubject=new f;createdPost$=this.createdPostSubject.asObservable();updatedPostSubject=new f;updatedPost$=this.updatedPostSubject.asObservable();insertSubject=new f;insert$=this.insertSubject.asObservable();updateSubject=new f;update$=this.updateSubject.asObservable();deleteSubject=new f;delete$=this.deleteSubject.asObservable();channel=k.channel("public:posts");constructor(){this.channel.on("postgres_changes",{event:"INSERT",schema:"public",table:"posts"},({new:t})=>{t?.id&&this.insertSubject.next({id:t.id,country_code:t.country_code??null,author_id:t.author_id??null})}).on("postgres_changes",{event:"UPDATE",schema:"public",table:"posts"},({new:t})=>{t?.id&&this.updateSubject.next({id:t.id,country_code:t.country_code??null,author_id:t.author_id??null,visibility:t.visibility??null})}).on("postgres_changes",{event:"DELETE",schema:"public",table:"posts"},({old:t})=>{t?.id&&this.deleteSubject.next({id:t.id,country_code:t.country_code??null,author_id:t.author_id??null})}).subscribe()}emit(t){this.createdPostSubject.next(t)}emitUpdated(t){this.updatedPostSubject.next(t)}emitDeleted(t){t?.id&&this.deleteSubject.next({id:t.id,country_code:t.country_code??null,author_id:t.author_id??null})}ngOnDestroy(){this.channel?.unsubscribe()}static \u0275fac=function(e){return new(e||d)};static \u0275prov=b({token:d,factory:d.\u0275fac,providedIn:"root"})};var O="demo_social_dataset_30k/posts.jsonl",j="demo_social_dataset_30k/comments.jsonl",E="demo_social_dataset_30k/video_captions.jsonl";function w(d){let t=d>>>0;return()=>{t|=0,t=t+1831565813|0;let e=Math.imul(t^t>>>15,1|t);return e=e+Math.imul(e^e>>>7,61|e)^e,((e^e>>>14)>>>0)/4294967296}}function S(d){let t=2166136261;for(let e=0;e<d.length;e++)t^=d.charCodeAt(e),t=Math.imul(t,16777619);return t>>>0}function B(d){let t=[],e=d.split(/\r?\n/);for(let o of e){let n=o.trim();if(n)try{t.push(JSON.parse(n))}catch{}}return t}var I=class d{constructor(t){this.fakeData=t}postsLoaded=!1;postsPromise=null;commentsLoaded=!1;commentsPromise=null;captionsLoaded=!1;captionsPromise=null;posts=[];postsById=new Map;postsByCountry=new Map;postsByAuthor=new Map;postStates=new Map;countryFeedCache=new Map;postMediaMeta=new Map;captionsByPost=new Map;commentsByPost=new Map;localCommentsByPost=new Map;commentStates=new Map;commentOrderCache=new Map;profileMap=null;countryOffsets=new Map;pexelsCache=new Map;pexelsInflight=new Map;async isDemoPostId(t){return await this.ensurePostsLoaded(),this.postsById.has(t)}isDemoCommentId(t){return/^cmt_|^local_/.test(t)}async listByCountry(t,e=25,o){o?.skipComments?await this.ensurePostsLoaded():await this.ensureCommentsLoaded();let n=String(t||"").trim().toUpperCase(),s=this.postsByCountry.get(n)??[],i=this.buildBalancedOrder(s,n),r=Math.max(1,e),a=this.sliceWithOffset(n,i,r);return await this.hydrateMedia(a),a}async listForAuthor(t,e=25){await this.ensureCommentsLoaded();let s=[...this.postsByAuthor.get(t)??[]].sort((i,r)=>new Date(r.created_at).getTime()-new Date(i.created_at).getTime()).slice(0,Math.max(1,e));return await this.hydrateMedia(s),s}async getPostById(t){await this.ensureCommentsLoaded();let e=this.postsById.get(t)??null;return e?(await this.hydrateMedia([e]),e):null}async listComments(t,e=25){await this.ensureCommentsLoaded();let o=this.getOrderedComments(t),n=this.localCommentsByPost.get(t)??[];return[...o,...n].slice(0,Math.max(1,e))}async addComment(t,e,o){await this.ensurePostsLoaded();let s=(await k.auth.getUser()).data.user?.id??"me",i=new Date().toISOString(),r={user_id:s,display_name:"You",username:null,avatar_url:null,country_name:null,country_code:null},a={id:`local_${Date.now()}_${Math.floor(Math.random()*1e4)}`,post_id:t,parent_id:o??null,author_id:s,body:e.trim(),like_count:0,liked_by_me:!1,created_at:i,updated_at:i,author:r},u=this.localCommentsByPost.get(t)??[];u.push(a),this.localCommentsByPost.set(t,u);let m=this.postStates.get(t);if(m){m.comment_count+=1;let _=this.postsById.get(t);_&&(_.comment_count=m.comment_count)}return a}async likePost(t){await this.ensurePostsLoaded();let e=this.postsById.get(t);if(!e)throw new Error("Post not found.");let o=this.ensurePostState(t);return o.liked_by_me||(o.liked_by_me=!0,o.like_count+=1),this.applyState(e,o),e}async unlikePost(t){await this.ensurePostsLoaded();let e=this.postsById.get(t);if(!e)throw new Error("Post not found.");let o=this.ensurePostState(t);return o.liked_by_me&&(o.liked_by_me=!1,o.like_count=Math.max(0,o.like_count-1)),this.applyState(e,o),e}async recordView(t){if(!t)return;await this.ensurePostsLoaded();let e=this.ensurePostState(t);e.view_count+=1;let o=this.postsById.get(t);o&&this.applyState(o,e)}async listLikes(t,e=25){await this.ensurePostsLoaded();let o=this.postStates.get(t),n=Math.max(0,Math.min(e,o?.like_count??0));if(!n)return[];let s=await this.fakeData.getProfiles();if(!s.length)return[];let i=w(S(`${t}|likes`)),r=new Set;for(;r.size<n&&r.size<s.length;)r.add(Math.floor(i()*s.length));let a=new Date().toISOString();return[...r].map(u=>{let m=s[u];return{user_id:m.user_id,created_at:a,user:this.profileToAuthor(m)}})}async likeComment(t){await this.ensureCommentsLoaded();let e=this.findComment(t);if(!e)throw new Error("Comment not found.");let o=this.ensureCommentState(t,e);return o.liked_by_me||(o.liked_by_me=!0,o.like_count+=1),e.liked_by_me=o.liked_by_me,e.like_count=o.like_count,e}async unlikeComment(t){await this.ensureCommentsLoaded();let e=this.findComment(t);if(!e)throw new Error("Comment not found.");let o=this.ensureCommentState(t,e);return o.liked_by_me&&(o.liked_by_me=!1,o.like_count=Math.max(0,o.like_count-1)),e.liked_by_me=o.liked_by_me,e.like_count=o.like_count,e}async ensurePostsLoaded(){if(!this.postsLoaded)return this.postsPromise?this.postsPromise:(this.postsPromise=(async()=>{await this.ensureCaptionsLoaded();let t=await this.fetchText(O),e=B(t),o=new Set;for(let n of e){let s=this.normalizeAuthorId(n?.author_id);s&&o.add(s)}await this.ensureProfileMap(o);for(let n of e){let s=this.normalizeAuthorId(n?.author_id);if(!n?.id||!s)continue;let i=this.profileMap?.get(s)??null,r=n.media?.type??"none",a=n.media?.url??null,u=n.media?.thumb_url??null,m=this.captionsByPost.get(n.id)??null,_=i?.country_code??n.country_code??null,h=i?.country_name??n.country_name??null,l=n.created_at||new Date().toISOString(),c={id:n.id,title:n.title??null,body:this.normalizeBody(n.body??"",h||n.country_name||null),media_type:r,media_url:a,thumb_url:u,media_caption:m,visibility:n.visibility||"public",like_count:0,comment_count:0,view_count:0,liked_by_me:!1,created_at:l,updated_at:l,author_id:s,country_name:h,country_code:_,city_name:null,author:i},g=this.seedCount(n.id,8e3),$=this.seedCount(`${n.id}|views`,18e4),v={like_count:g,comment_count:0,view_count:Math.max($,g*6),liked_by_me:!1};this.postStates.set(n.id,v),this.applyState(c,v),this.posts.push(c),this.postsById.set(n.id,c);let p=String(_||"").trim().toUpperCase();if(p){let M=this.postsByCountry.get(p)??[];M.push(c),this.postsByCountry.set(p,M)}let C=this.postsByAuthor.get(s)??[];C.push(c),this.postsByAuthor.set(s,C),this.postMediaMeta.set(n.id,{type:r,query:n.media?.query??null,url:a,thumb_url:u})}this.postsLoaded=!0})(),this.postsPromise)}async ensureCommentsLoaded(){if(!this.commentsLoaded)return this.commentsPromise?this.commentsPromise:(this.commentsPromise=(async()=>{await this.ensurePostsLoaded();let t=await this.fetchText(j),e=B(t),o=new Set;for(let n of e){let s=this.normalizeAuthorId(n?.author_id);s&&o.add(s)}await this.ensureProfileMap(o);for(let n of e){let s=this.normalizeAuthorId(n?.author_id);if(!n?.id||!n?.post_id||!s)continue;let i=this.profileMap?.get(s)??null,r=n.created_at||new Date().toISOString(),a={id:n.id,post_id:n.post_id,parent_id:n.parent_id??null,author_id:s,body:n.body??"",like_count:this.seedCount(n.id,160),liked_by_me:!1,created_at:r,updated_at:r,author:i},u=this.commentsByPost.get(n.post_id)??[];u.push(a),this.commentsByPost.set(n.post_id,u),this.commentStates.set(n.id,{like_count:a.like_count,liked_by_me:!1});let m=this.postStates.get(n.post_id);m&&(m.comment_count+=1)}for(let[n,s]of this.postStates.entries()){let i=this.postsById.get(n);i&&(i.comment_count=s.comment_count)}this.commentsLoaded=!0})(),this.commentsPromise)}async ensureCaptionsLoaded(){if(!this.captionsLoaded)return this.captionsPromise?this.captionsPromise:(this.captionsPromise=(async()=>{let t=await this.fetchText(E),e=B(t);for(let o of e)!o?.post_id||!o?.caption||this.captionsByPost.set(o.post_id,o.caption);this.captionsLoaded=!0})(),this.captionsPromise)}async ensureProfileMap(t){if(!this.profileMap){let e=await this.fakeData.getProfiles(),o=new Map;for(let n of e)o.set(n.user_id,this.profileToAuthor(n));this.profileMap=o}if(t&&t.size){await this.fakeData.ensureProfilesById(t);for(let e of t){if(this.profileMap?.has(e))continue;let o=await this.fakeData.getProfileById(e);o&&this.profileMap?.set(e,this.profileToAuthor(o))}}}profileToAuthor(t){return{user_id:t.user_id,display_name:t.display_name??null,username:t.username??null,avatar_url:t.avatar_url??null,country_name:t.country_name??null,country_code:t.country_code??null}}applyState(t,e){t.like_count=e.like_count,t.comment_count=e.comment_count,t.view_count=e.view_count,t.liked_by_me=e.liked_by_me}ensurePostState(t){let e=this.postStates.get(t);return e||(e={like_count:0,comment_count:0,view_count:0,liked_by_me:!1},this.postStates.set(t,e)),e}ensureCommentState(t,e){let o=this.commentStates.get(t);return o||(o={like_count:e.like_count,liked_by_me:e.liked_by_me},this.commentStates.set(t,o)),o}findComment(t){for(let e of this.commentsByPost.values()){let o=e.find(n=>n.id===t);if(o)return o}for(let e of this.localCommentsByPost.values()){let o=e.find(n=>n.id===t);if(o)return o}return null}seedCount(t,e){let o=w(S(t));return Math.floor(Math.pow(o(),2)*e)}buildBalancedOrder(t,e){let o=Date.now(),n=Math.floor(o/(1e3*60*15)),s=`${e}:${n}`,i=this.countryFeedCache.get(s);if(i)return i.posts;let r=[...t].sort((l,c)=>new Date(c.created_at).getTime()-new Date(l.created_at).getTime()),a=new Map;for(let l of r){let c=a.get(l.author_id)??[];c.push(l),a.set(l.author_id,c)}let u=Array.from(a.keys()),m=w(S(`${e}|${n}`));for(let l=u.length-1;l>0;l-=1){let c=Math.floor(m()*(l+1)),g=u[l];u[l]=u[c],u[c]=g}let _=[],h=!0;for(;h;){h=!1;for(let l of u){let c=a.get(l);!c||!c.length||(_.push(c.shift()),h=!0)}}return this.countryFeedCache.set(s,{ts:o,posts:_}),_}sliceWithOffset(t,e,o){if(!e.length)return[];if(e.length<=o)return[...e];let s=(this.countryOffsets.get(t)??0)%e.length,i=s+o,r=(s+o)%e.length;if(this.countryOffsets.set(t,r),i<=e.length)return e.slice(s,i);let a=e.slice(s),u=e.slice(0,i-e.length);return[...a,...u]}normalizeBody(t,e){let o=String(t||"").trim();if(!o)return"";let n=o,s=/^in\s+[A-Za-z][^,.:;-]{1,60}[,.:;-]\s*/i;if(n=n.replace(s,"").trim(),!n||!e)return n;let i=e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),r=new RegExp(`^In\\s+${i}\\b[\\s,.-]*`,"i");return n.replace(r,"").trim()}getOrderedComments(t){let e=this.commentOrderCache.get(t);if(e)return e;let o=this.commentsByPost.get(t)??[];if(!o.length)return this.commentOrderCache.set(t,[]),[];let n=[],s=new Set;for(let r of o){let a=`${r.author_id}|${this.normalizeCommentBody(r.body)}`;s.has(a)||(s.add(a),n.push(r))}let i=w(S(`${t}|comments`));for(let r=n.length-1;r>0;r-=1){let a=Math.floor(i()*(r+1)),u=n[r];n[r]=n[a],n[a]=u}return this.commentOrderCache.set(t,n),n}normalizeCommentBody(t){return String(t||"").trim().toLowerCase().replace(/\s+/g," ")}normalizeAuthorId(t){let e=String(t||"").trim();if(!e)return null;let o=e.match(/^user_(\d+)$/i);if(!o)return e;let n=parseInt(o[1],10);return!Number.isFinite(n)||n<=0?e:`user_${String(n).padStart(6,"0")}`}resolveAssetUrl(t){let e=document.querySelector("base")?.getAttribute("href")??"/",o=new URL(e,window.location.origin).toString();return new URL(t,o).toString()}async fetchText(t){if(typeof window>"u"||typeof document>"u")return"";let e=this.resolveAssetUrl(t),o=await fetch(e);if(!o.ok)throw new Error(`Failed to load ${t}: ${o.status}`);return o.text()}async hydrateMedia(t){if(!y.pexelsApiKey)return;let e=t.map(async o=>{if(!o||o.media_type==="none"||o.media_url)return;let n=this.postMediaMeta.get(o.id);if(!n||!n.query)return;let s=`${n.type}:${n.query}`.toLowerCase(),i=this.pexelsCache.get(s);if(i){o.media_url=i.url,o.thumb_url=i.thumb_url;return}let r=this.pexelsInflight.get(s);if(r){let m=await r;o.media_url=m.url,o.thumb_url=m.thumb_url;return}let a=this.fetchPexels(n.type,n.query);this.pexelsInflight.set(s,a);let u=await a;this.pexelsInflight.delete(s),this.pexelsCache.set(s,u),o.media_url=u.url,o.thumb_url=u.thumb_url});await Promise.all(e)}async fetchPexels(t,e){let o=y.pexelsApiKey||"";if(!o)return{url:null,thumb_url:null};let n={Authorization:o};if(t==="video"){let m=`https://api.pexels.com/videos/search?query=${encodeURIComponent(e)}&per_page=1`,_=await fetch(m,{headers:n});if(!_.ok)return{url:null,thumb_url:null};let l=(await _.json())?.videos?.[0];if(!l)return{url:null,thumb_url:null};let g=(Array.isArray(l.video_files)?l.video_files:[]).filter(p=>String(p?.file_type||"").toLowerCase()==="video/mp4").sort((p,C)=>(p?.width??0)-(C?.width??0)),$=g.find(p=>(p?.width??0)>=720)||g[0],v=l.video_pictures?.[0]?.picture||l.image||null;return{url:$?.link??null,thumb_url:v}}let s=`https://api.pexels.com/v1/search?query=${encodeURIComponent(e)}&per_page=1`,i=await fetch(s,{headers:n});if(!i.ok)return{url:null,thumb_url:null};let u=(await i.json())?.photos?.[0]?.src||{};return{url:u?.large??u?.medium??null,thumb_url:u?.medium??null}}static \u0275fac=function(e){return new(e||d)(P(q))};static \u0275prov=b({token:d,factory:d.\u0275fac,providedIn:"root"})};var A=class d{constructor(t,e,o){this.gql=t;this.postEvents=e;this.demoData=o}async listByCountry(t,e=25,o){if(y.useDemoDataset){let i=o?.demoLimit??1e3,r=Math.max(e,i),[a,u]=await Promise.allSettled([this.withTimeout(this.gql.request(`
          ...
          `,{code:t,limit:r}),1600,"postsByCountry"),this.demoData.listByCountry(t,r,{skipComments:o?.skipComments})]),m=a.status==="fulfilled"?(a.value.postsByCountry??[]).map(h=>this.mapPost(h)):[],_=u.status==="fulfilled"?u.value:[];return this.mergePosts(m,_,r)}let n=`
      query PostsByCountry($code: String!, $limit: Int) {
        postsByCountry(country_code: $code, limit: $limit) {
          id
          title
